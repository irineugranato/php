...
<body>
  ...
  $sql = "SELECT id, colaborador, dia, horario, local FROM entrada";
  $result = $link->query($sql);
	...
	echo "<hr>";
	echo "<br> ";
	echo "<h2>Gestão de Pontos - Entradas</h2>";
	...
	echo " <tbody id='myTable'> ";
				if ($result->num_rows > 0) {
					...
					while($row = $result->fetch_assoc()) {
						...
						echo "<td><a href='deleteEntrada.php?id=".$row["id"]."' onclick='return delAlert(".$row["id"].");'>Remover</a></td>";
						echo "</tr>";			
					}
				} else {
					echo "0 resultados";
